...
Pre-hackathon implementation phase **complete**; first real-API verification run executed 2026-05-04, layout-calibration regression on the Seibido 9104 sample fixed and re-verified end-to-end on 2026-05-05. Lineup-column OCR + per-batter spreadsheet ground-truth verification landed 2026-05-09 — `batting_order → player name` mapping works end-to-end, alignment is correct on the verified game, and the active accuracy bottleneck is recognition of handwritten **diamond-edge tracing** as the hit indicator (hit recall 20% on the verified game while walks / strikeouts recall hold at 75 / 67%).
...
- [x] **265 tests pass, 7 skipped** on the GitHub Codespace (`bun run test`). The 7 skips are the Hough WASM integration tests that opt in via `RUN_WASM_TESTS=1`; the underlying module is instead covered by a pure-Node smoke script (see the Day 2 pre-work bullet below). Typecheck is clean (`bun run typecheck`), and `DRY_RUN=1 bun run pipeline` on a sample image completes with the expected 14 planned Claude calls and zero API cost.
...
- [x] **Day 2 pre-work (2026-04-19 evening, API-free)** — Filled three Day 2 prerequisites while the real-API verification decision is still pending. (1) **Keio / Chiba prompts promoted from stubs to production builders** (`5354b92` + `c64ab8c`, 27 tests added). `keio-fewshot.ts` now carries eight keio-specific examples — `BB` walk, `SO` for swinging strikeout, the keio-only `K` for *dropped-third-strike REACHED* (the single exception where `outcome=strikeout_swinging` and `extras.strikeout_reached=true` legitimately coexist), `O`+base for stolen bases, and the ◇-boxed-fielder = infield-hit convention that is the direct inverse of waseda's sacrifice-bunt reading. `chiba-fewshot.ts` carries three examples under an explicit "Day-1 stub + waseda fallback + low-confidence on chiba-specific marks" policy. `buildKeioSystemPrompt(fewshot)` / `buildChibaSystemPrompt(fewshot)` mirror `buildWasedaSystemPrompt` so `stage2-extract-cells::selectSystemPrompt` dispatches all three schools through the same builder path. The `WasedaFewshotExample` type now carries an optional `alternatives?: string[]` field so any example with `confidence < 0.7` can demonstrate the ≥2-alternatives invariant the Zod schema enforces (previously the chiba unknown example would have taught the model to omit it — caught by sub-agent review and fixed before merge). (2) **`validate.ts` rule #4 `diamond_reached_base_mismatch`** (`be1c790` + `4dd1ff3`, 12 tests added). `reached_base = 4` (scored) is only legal when one of four things holds: `outcome === "home_run"`, a subsequent at-bat exists in the same inning, `extras.stolen_bases.includes(4)` (stolen home specifically — 2B/3B alone can't score without later events), or `extras.wild_pitch` / `passed_ball` / `error_fielder` is set. Anything else is warned as a likely diamond-shading mis-read. This lifts `validate.ts` coverage from 6/8 to **7/8 of the §6.1 rules** — the remaining three (`runs_total_mismatch`, `pitcher_totals_mismatch`, `at_bats_mismatch`) need totals-row / pitcher-log OCR and are Day 2 work. Both sub-agent reviews flagged the initial `stolen_bases.length > 0` guard as too loose — tightened to `.includes(4)` with a regression test. (3) **`@techstark/opencv-js` Hough line detector merged to main**. `src/preprocess/hough-calibrate.ts` exposes `getOpenCV()` as a lazy WASM singleton, `detectGridLines()` (sharp downscale → grayscale → Canny → `HoughLinesP` with angle-based horizontal / vertical classification and Mat memory hygiene via try/finally `delete()`), and `clusterVerticalBoundaries()` (line-length-weighted non-maxima suppression that picks N column-boundary x coordinates). The module is intentionally decoupled from `cropInnings` — it becomes the Day 2 calibration step for camera-shot inputs where the static Seibido ratios don't fit due to perspective or rotation. **Key finding from this session's verification**: `@techstark/opencv-js` is a UMD Emscripten module whose `default` export is a thenable whose `.then` hook doesn't actually resolve through Node's ESM dynamic-import path — `await import(...)` silently deadlocks, and even `createRequire(import.meta.url)` from a `.ts` file gets intercepted by Vite's test-runner loader and hits the same deadlock. The working pattern is a pure `.cjs` helper (`src/preprocess/opencv-loader.cjs`) that calls `require("@techstark/opencv-js")` outside the transformer graph and resolves on the classic `cv.onRuntimeInitialized` callback. A pure-Node smoke script (`scripts/hough-smoke.cjs`) exercises init → grayscale → Canny → `HoughLinesP` end-to-end in **~706 ms median of 3 cold-start runs on the Codespace host** (588 / 706 / 765 ms; init dominates at ~500 ms, detection ~50 ms), confirming the implementation itself. Numbers are environment-dependent; reproduce with `bun run smoke:hough`. The in-process WASM tests inside the test runner stay deadlocked regardless of `deps.inline` / `pool: forks` / `singleFork` tuning, so they're gated behind `RUN_WASM_TESTS=1` via `describe.skipIf` and are expected to be exercised either in a Day-2 integration harness or via the smoke script. `clusterVerticalBoundaries` also had its `targetCount >= 2` guard relaxed to `>= 1` so asking for a single dominant peak (the noise-suppression test case) is valid. **Test count lifted 219 → 265 on main, plus 7 WASM tests opt-in.**
- [x] **Day 2 PWA scaffold — Phase 1** (2026-04-19) — Next.js 15 App Router + `@serwist/next` v9 + Dexie 4 + UUID v7 wired up without touching the backend OCR pipeline. `next.config.ts` registers Serwist against `src/pwa/sw.ts`, which installs a `BackgroundSyncQueue` (`scorebook-events-queue`, 24h retention) for `POST /api/events` so offline plate-appearance submissions are replayed on reconnect — the architecture §9.2 SW layer of the two-layer queue. `src/client/db/dexie.ts` declares the six-store schema from §8.2 (`games / gameEvents / pendingEvents / teams / teamMembers / players`) with the `[gameId+seq]` compound index that mirrors the Supabase `UNIQUE (game_id, seq)` constraint. `src/client/db/uuid-v7.ts` wraps `uuid@11`'s `v7()` so every client-generated `event.id` is globally time-sortable per §8.1. The root `app/` route includes a minimal `PwaStatus` client component that surfaces online state + Dexie row counts as a smoke-check surface.
- [x] **Day 2 PWA Phase 2 — Supabase foundation** (2026-04-19, `24ecd12` → `7ad4393` → `5d6ace9`). `@supabase/ssr@0.5.2` + `@supabase/supabase-js@2.103.3` added. `src/client/supabase/{client,server,middleware,realtime}.ts` + root `middleware.ts` implement session refresh with `/api/events` / SW shell / static assets excluded from the matcher footgun. Realtime **Broadcast** subscribe helper unwraps `payload.record` (the DB trigger fires `realtime.send(jsonb_build_object('record', row_to_json(new)), 'INSERT', 'game:'+gameId, false)`) into typed envelopes. `src/client/sync/envelope-convert.ts` provides four symmetric conversions (envelope ↔ Dexie row ↔ Supabase row) covering `null` ↔ `undefined` + camelCase ↔ snake_case, 11 pure-function test cases. `src/client/sync/pending-flush.ts` is the Safari-fallback flush with a `maxRetryCount=10` poison gate and `getAuthToken()` Bearer header. `supabase/migrations/0001_init.sql` (~440 lines) carries the full §8.3 schema: `teams / team_members / invitations / games / game_events / players`, the `redeem_invitation()` RPC with per-user 10-minute / 5-attempt rate limit, the §9.1 `broadcast_changes` trigger on `game_events`, and §10 RLS policies wrapping `(select auth.uid())` with `to authenticated` and `SECURITY DEFINER` helpers (`user_team_ids(user_id)` / `has_team_role(team_id, user_id, role)`) so the policy shape lands in the "~2 ms" bucket of the official Supabase benchmark rather than the ">2 min" one. `app/api/events/route.ts` is a 501 placeholder with the two auth-decoding paths documented in the header (middleware is intentionally excluded from this route so the handler must verify the Bearer token itself — this is a Day-2 implementation slot). Opus sub-agent review surfaced Blocker 2 / Major 6 / Minor 6 / Nit 3; Blockers 1 + 2 and the high-impact Majors were all fixed before merge. **B1**: `prevent_last_owner_leaving` now takes `pg_advisory_xact_lock(hashtext(team_id::text))` with DELETE / UPDATE triggers split, closing the parallel-leave race where two owners could both pass the `COUNT(*) > 1` check and leave the team ownerless. **B2**: the `game_events_insert` RLS policy wrapped its `team_id` lookup in a new `current_user_can_write_game(game_id)` `SECURITY DEFINER STABLE` helper, eliminating the scalar subquery that re-planned per row, and the table now explicitly `REVOKE UPDATE, DELETE ON game_events FROM authenticated` to make the append-only contract enforceable at the privilege layer instead of just the policy layer. **M4 / M5**: pending-flush grew the retry-count cap + Bearer authorization header. **M7**: `team_members.user_id` is `ON DELETE RESTRICT` (history integrity — deleting an auth user must not silently erase their appearance in game_events). **M8**: the realtime helper drops malformed payloads with a warn instead of falling back to the raw `row` shape. The review also flagged M6 (broadcast signature), which turned out to be a reviewer mis-read — `realtime.send(payload, event, topic, private)` is the documented Supabase signature, so that one stayed as-is.
- [x] **Day 2 PWA Phase 3 — UI placeholder routes** (2026-04-19, `ecfc6b0` → `37a8fe4`, +1,026 lines / 12 files). Five routes seeded so Day-2-proper can focus on wiring logic, not on building screens from scratch. **`/login`** is email-magic-link sign-in — `signInWithOtp` with `emailRedirectTo: ${window.location.origin}/games`, four-state machine (`idle / submitting / sent / error`). **`/join?code=XXX`** redeems an 8-character invite via `supabase.rpc('redeem_invitation', { p_code: code })`, monospace input, the code pre-fills from the URL for LINE-share flow. **`/games`** lists Dexie-backed games (`db.games.orderBy("date").reverse().toArray()`) with a `STATUS_LABEL: Record<GameRow["status"], string>` (`in_progress / finished / suspended`) and nav to `/capture` / `/join`. **`/games/[gameId]`** renders a 9-inning × 9-batter skeleton grid with a sticky-left-column batter header; the placeholder hard-codes `DEFAULT_LINEUP = 9` and flags §20.3's 9-11 variable batter count as a Day-2 decision between "infer from `gameEvents` substitution accumulation" vs "add `GameRow.batterCount`". **`/capture`** is the camera skeleton — `navigator.mediaDevices.getUserMedia` with the §11.3 iOS-safe constraint (`facingMode: { ideal: 'environment' }`, never `{ exact }`), two `useEffect`s for `srcObject` binding and `MediaStreamTrack.stop()` cleanup, shutter button disabled pending Day-2 jscanify + OpenCV.js integration. All five pages use Next.js 15's Promise-based `params` / `searchParams` with server-component page + `"use client"` child split. `tryGetSupabaseBrowserClient()` is a non-throwing variant added so routes render without env vars present — the original throwing version still backs production auth flows. `app/page.tsx` gains a nav-card list linking to all five routes. Opus sub-agent review returned Blocker 0 / Major 8 (7 verified-OK / 1 actual fix) / Minor 5 / Nit 2 → **ship verdict**. The one fix: the `/games/[gameId]` placeholder comment referenced `game.lineup`, which doesn't exist on `GameRow` (the actual schema in `src/client/db/dexie.ts:22-30` has `id / teamId / date / opponent / status / createdAt / updatedAt`) — rewritten to point at the two real Day-2 options. Codespace `bun run typecheck` clean, `bun run test` **276 pass / 7 skipped**, `bun run build` succeeds with 9 routes (`/` + `/login` + `/games` + `/capture` static, `/games/[gameId]` + `/join` + `/api/events` dynamic, + Middleware 87.2 kB).
- [x] **Cross-host smoke validation on RPi5 (2026-05-03)** — `scripts/hough-smoke.cjs` re-runs in **365 ms** on a Raspberry Pi 5 (Cortex-A76 / 8 GB / Tailscale-only ingress), confirming the OpenCV WASM path is portable beyond the original Codespace host. The 20-image private fixture set (gitignored, real-game scans with player names) was relocated from the Codespace `data/samples/` to `~/scorebook-private/samples/` on the RPi5 so Day-2 calibration work can run against real inputs without keeping a Codespace warm.
- [x] **First real API verification run completed (2026-05-04).** Single-column run against `inning_01.png` from `scripts/hough-snap-prior.cjs` — **$0.2874 / 16.9 s / attempt 1**, `cache_creation_input_tokens = 6549` warming the system prompt for the 13-column game-level run that follows. The Stage 2 Zod schema parsed the response cleanly and the result was upserted into `data/scorebook.smoke.db` (`ocr_runs.id = 2`). Flagged in the per-cell `evidence` field: the crop appeared to contain "kanji surnames like 谷川, 藤, 橋, 峰" rather than at-bat marks, surfacing a layout-calibration regression on this specific Seibido 9104 scan — tracked into the 2026-05-05 work below.
- [x] **2026-05-03 direction pivot — RPi5-local SQLite + CSV is the active write target.** Per explicit user feedback ("the site doesn't matter, OCR getting the data right and writing it does, prefer Raspberry Pi 5 over Codespace"), the PWA / Next.js / Supabase Auth / RLS work froze on `main` for reference and is no longer on the active task list. The OCR pipeline now writes directly into a SQLite database on the Raspberry Pi 5 (Tailscale-only ingress) and exports to long-format / wide-format CSV for downstream analysis. Architecture document and the priority list at the bottom of this README are kept as historical context for an eventual reactivation; the live task plan is in the project's pickup-queue memory file.
- [x] **2026-05-04 — SQLite v0 schema + CSV exporter + Hough snap-to-prior calibration** (Tasks #5 / #6 / #8 of the post-pivot plan). `data/migrations/0001_init.sql` ships the five-table per-game schema (`games / players / cells / ocr_runs / crop_jobs`) with the `(game_id, batting_order, inning)` UNIQUE upsert key, the `cells.outcome` enum aligned to architecture §6.1, and an `idx_cells_low_confidence` partial index on `confidence < 0.7` so review-queue scans stay cheap as the cell table grows. `data/fixtures/0001_smoke.sql` seeds a one-game smoke (9 batters, 10 cells, one deliberately low-confidence cell to exercise the partial index). `scripts/export-csv.sh` writes long-format and wide-format (one column per inning, hardcoded 13 CASE expressions) CSV via `sqlite3 .mode csv`. `scripts/hough-snap-prior.cjs` upgrades the prior-only fixed-N-equal column splitter into a snap-to-prior algorithm — fixed N-equal stays as the prior, `±25 % inningWidth` is the search window for `cv.HoughLinesP` peaks, snapping is line-length-weighted across multiple candidates, and prior fallback fires when no peak is found inside the window. On the `241201_1.jpg` sample, the algorithm settles all 14 boundaries (snap 6 / prior fallback 8) in ~3.1 s on the RPi5, max delta 13 px from the equal-spaced prior — guaranteeing a complete column set even when half of the printed grid lines fail Canny.
- [x] **2026-05-05 — layout re-calibration + end-to-end legibility-improvement verification** (Tasks #9–#12). The 2026-05-04 run flagged that `playerColRatio = 0.135` clipped player-name column content into the `inning_01` crop on this Seibido 9104 scan. A Hough vertical-line search inside `x ∈ [200, 600]` of the normalized 2576 px landscape (`scripts/calibrate-player-col.cjs` + `scripts/calibrate-strip.cjs`) plus visual overlay verification confirmed the printed inning-grid left edge sits at `x = 505` (3 candidates, weighted-average snap), corresponding to **`playerColRatio = 0.196` and `rightStatsRatio = 0.812`** (the right edge similarly snapped to a 1-candidate Hough peak at `x = 2083`). The constants were propagated to `src/types/layout.ts` and the four `scripts/*` consumers (`*.bak-cal` backups retained). Verification: a second real-API call at the new ratios returned `column_quality.legibility = 0.7` (was 0.4 with the original ratios), with `inning_01` decoded as `IK` (strikeout swinging, conf 0.9) / `IIF4` (fly out to 2B, conf 0.85) / `III2` (pop out to catcher, conf 0.7) and batters 4–10 correctly empty (conf 1.0) — matching the visible Roman-numeral progression on the original page. Cost: **$0.2945 / 17.9 s**. The result was upserted into `data/scorebook.smoke.db` using a `CREATE TEMP TABLE _new_run_id AS SELECT last_insert_rowid()` capture pattern after discovering that `last_insert_rowid()` re-evaluates per row inside an `INSERT … SELECT`, which had silently skewed the `ocr_run_id` linkage in the first INSERT attempt. The duplicate `ocr_runs.id = 3` row left over from a 2026-05-04 sqlite3-CLI partial-commit (the migration file was missing `.bail on`) was cleaned up in the same migration set.
- [x] **2026-05-09 — lineup column OCR + layout v2 (`playGridBottom 0.520 → 0.700`) + per-batter spreadsheet ground-truth verification** (Tasks #13–#16). Added the player-name column extraction pipeline: `scripts/name-col.cjs` crops the left strip (`x ∈ [0, playerColRatio · W]`, full play-grid Y range) and `scripts/stage2-name-col.ts` (a Sonnet-4.6 driver mirroring `stage2-one-column.ts` but with an inline `extract_name_column` tool schema and a system prompt that explicitly distinguishes the printed slot-marker numbers `11/12/13/...` from the handwritten jersey numbers + surnames). The lineup pass on the 2024-12-01 sample resolves all 10 starters + 1 substitute (1 平野 / 2 盛島 / 3 水野(暢) / 4 奥秋(佑) → sub 佐藤 / 5 村田 / 6 野口(亮) / 7 安岡 DH / 8 森川 / 9 江島 / 10 土屋), with one kanji-disambiguator miss (`亮 → 亘`). **Layout fix #2** (`playGridBottom 0.520 → 0.700`): the original ratio covered only ~7 of the 10 batter rows, so equal-division of the inning column into 10 row crops silently displaced batters 8–10 into the bottom 3 slots and the resulting cell readings drifted across batting orders. Bumped to `0.700` and re-verified on `241201_2`; v2 alignment passes 平野's 2 BB / 奥秋's 1 BB / 森川's 2 K all on the correct `batting_order` rows. **Codespace / RPi5 calibration-drift fix**: the Codespace `hough-snap-prior.cjs` was still on `playerColRatio = 0.135 / rightStatsRatio = 0.770` from a `.bak` revert, which had moved the inning-1 boundary into the player-name column and let the model OCR `"盛 島"` / `"水 野(柳)"` etc. as inning-1 cell content. Synced to the RPi5 values (`0.196 / 0.812`), inning-1 boundary `x = 348 → 532`, contamination resolved. Added `scripts/stage2-inning-sonnet.ts` — a Sonnet-4.6 variant of `stage2-one-column.ts` (uses `claude-sonnet-4-6` directly via inline `Anthropic` SDK calls instead of going through `client.ts`'s `DEFAULT_MODEL = claude-opus-4-7`), so the full 13-inning re-OCR drops from a projected ~$3 (Opus) to **~$0.51 (Sonnet, cache hits after the first call)**. Per-batter spreadsheet verification on the re-OCR run reports **walks recall 75% / precision 50%, strikeouts recall 67% / precision 50%, hits recall 20% / precision 67%** — alignment is correct, but the model is failing to recognise handwritten **diamond-edge tracing** as the hit indicator. Visual confirmation on the 3rd batter's four plate appearances (3 H + 1 BB + 0 K + 5 RBI in this game): inning 1 = walk (four ball-count dots, no diamond trace), inning 2 = home run (all four diamond edges traced + `C` mark inside), inning 4 = single (right-edge trace + `○` reached-base mark + `(7)` LF), inning 7 = likely the triple. The OCR reads the position-number annotation `(n)` but treats the lightly-printed dotted diamond and the heavy hand-traced perimeter as equivalent, defaulting all hits to `ground_out` / `unknown`. The `docs/scorebook-notation.md` reference does encode "filled diamond side" as a hit indicator and `src/ocr/prompts/waseda-system.ts` does describe diamond shading, but `src/ocr/prompts/waseda-fewshot.ts` only carries one `1B` example and no example of the `(n) + traced edge → single` synthesis the actual scorebook uses. **Active next step**: image-based fewshot built from the confirmed water cells (i2 HR / i4 single / i1 BB / i7 triple after user ground-truth confirmation) + a cell-level crop pipeline (each cell up-sampled to ≥600 px on its own instead of an inning column to ≥600 px) so the vision encoder has enough resolution to discriminate hand-traced ink from printed template. Cumulative real-API spend across the 2026-05-04 / 05 / 06 / 09 sessions touching this work ≈ **$0.69**.
