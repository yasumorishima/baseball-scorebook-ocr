...
- [x] **175 tests pass** on the GitHub Codespace (`bun run test`), typecheck is clean (`bun run typecheck`), and `DRY_RUN=1 bun run pipeline` on a synthetic canvas completes with the expected 14 planned Claude calls and zero API cost.
- [x] **Sub-agent review** — A second Opus 4.7 instance reviewed phases A–E. Blocker count: 0. Two majors were surfaced and fixed in the same session: the `event.ts` `GameEventType` enum now matches the Supabase `game_events` schema (`plate_appearance / substitution / correction / inning_end / game_end` with a `correctionOf` envelope field), and `compute.ts` now treats an explicit `extras.SH / SF / HBP` flag as authoritative over the `outcome` branch so a Claude response combining e.g. `outcome=ground_out` with `extras.SH=true` no longer double-counts the plate appearance. Regression tests were added.
